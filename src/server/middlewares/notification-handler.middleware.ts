...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'Family member addition.create': {
    roles: ['tenant-roles', 'tenant-owner-roles'],
    key: 'tenantRoles',
    tenantPath: [],
    userPath: [],
  },
  'Family member removal.delete': {
    roles: ['tenant-roles', 'tenant-owner-roles'],
    key: 'tenantRoles',
    tenantPath: [],
    userPath: [],
  },
  'New customer request.create': {
    roles: ['tenant-roles', 'tenant-owner-roles'],
    key: 'customerRoles',
    tenantPath: [],
    userPath: [],
...
const tenantRoles: string[] = ['owner', 'family-member'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.family.findFirst({
