...
import { getSavings } from 'apiSdk/savings';
import { SavingsInterface } from 'interfaces/savings';

import { BudgetListPage as ShowCaseList } from 'pages/budgets';
...
function DashboardPage() {
  ...
  const savingsStat = useSWR<PaginatedInterface<SavingsInterface>>(
    () => `/savings`,
    () => getSavings({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Savings',
      result: savingsStat?.data?.totalCount,
      isLoading: savingsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`budgets`} passHref legacyBehavior>
                  <Button
